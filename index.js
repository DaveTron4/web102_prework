...
function addGamesToPage(games) {
    ...
    for(let i = 0; i < games.length; i++) {
        ...
        <p><strong>Goal:</strong> $${game.goal.toLocaleString('en-US')}</p>
...
function filterUnfundedOnly() {
    ...
    const filteredUnfunded = GAMES_JSON.filter( (game) => game.pledged < game.goal);

    // use the function we previously created to add the unfunded games to the DOM
    addGamesToPage(filteredUnfunded);
}
...
function filterFundedOnly() {
    ...
    const filteredFunded = GAMES_JSON.filter( (game) => game.pledged >= game.goal);
...
Currently ${totalUnfunded} ${totalUnfunded === 1 ? 'game remains' : 'games remain'} unfunded. We need your help to fund these amazing games!
`
...
// firstGame.classList.add('top-funded')
